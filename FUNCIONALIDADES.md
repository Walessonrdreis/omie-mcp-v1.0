...
| Consultar Extrato de Conta Corrente | Mostra o extrato de uma conta (banco, caixa, cartão) num período: cada movimento (data, descrição, valor, categoria, se já foi conciliado) e os saldos (anterior, atual, conciliado, disponível). |
...
| Gerar/Obter/Prorrogar/Cancelar Boleto | Gera o boleto de um título de contas a receber, busca o link de um já gerado, adia o vencimento ou cancela. ⚠️ Exige convênio bancário configurado na conta Omie — sem isso a Omie recusa a geração. |
| Listar/Obter/Gerar/Cancelar PIX | Lista os PIX gerados (com status), busca o QR Code/copia-e-cola de um título específico, gera um novo PIX de cobrança ou cancela um pendente. |
| Consultar Orçamento de Caixa | Mostra o orçamento de caixa **nativo da Omie** (previsto x realizado) por categoria financeira, num mês/ano — complementa o fluxo de caixa calculado pelo MCP. |
